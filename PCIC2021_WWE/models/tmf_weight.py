...
from models.loss import TMFWLoss
...
class Objective:
    ...
    def __init__(self, num_users, num_tags, train, valid, test, rating, item2tag, iters, seed, save) -> None:
        """Initialize Class"""
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        # alpha = trial.suggest_uniform('alpha', 0.01, 0.99)

        rank = trial.suggest_categorical('rank', [56])
        ...
        alpha = trial.suggest_categorical('alpha', [0.030273056366871])

        setup_seed(self.seed)

        model = TMFW(self.num_users, self.num_tags, np.int(rank), np.int(batch_size), lamb=lam, learning_rate=lr,
                     alpha=alpha).cuda()

        score, _, _, _, _ = model.fit(self.train, self.valid, self.test, self.rating, self.item2tag, self.iters,
                                      self.seed, self.save)
...
class Tuner:
    ...
    def tune(self, n_trials, num_users, num_tags, train, valid, test, rating, item2tag, num_epoch, seed, save):
        """Hyperparameter Tuning by TPE."""
        objective = Objective(num_users=num_users, num_tags=num_tags, train=train, valid=valid, test=test,
                              rating=rating, item2tag=item2tag, iters=num_epoch, seed=seed, save=save)
...
class TMFW(nn.Module):
    ...
    def fit(self, matrix_train, matrix_valid, matrix_test, matrix_rating, item2tag, num_epoch=100, seed=0, save=False):
        setup_seed(seed)
        ...
        for epoch in tqdm(range(num_epoch)):
            dataloader_iterator = iter(tag_train_dataloader)

            for i, rating_data in enumerate(rating_train_dataloader):
                tag_data = next(dataloader_iterator)
...
def tmfw(matrix_train, matrix_valid, matrix_test, matrix_rating, item2tag, matrix_utrain=None, iteration=100, lam=0.01,
         rank=200, batch_size=500, learning_rate=1e-3, seed=0, alpha=0.1, source=None, searcher='grid', n_trials=1,
         save=True, **unused):
    progress = WorkSplitter()

    progress.section("TMF-Weight: Set the random seed")
    setup_seed(seed)

    progress.section("TMF-Weight: Training")
    ...
    if searcher == 'optuna':
        ...
        trials, best_params = tuner.tune(n_trials=n_trials, num_users=m, num_tags=n, train=matrix_input,
                                         ...
                                         num_epoch=iteration, seed=seed, save=save)
        return trials, best_params

    if searcher == 'grid':
        model = TMFW(m, n, rank, batch_size, lamb=lam, learning_rate=learning_rate, alpha=alpha).cuda()

        _, U, V, uB, vB = model.fit(matrix_input, matrix_valid, matrix_test, matrix_rating, item2tag, iteration, seed)
